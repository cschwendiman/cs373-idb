...
def main():
    oauth = get_oauth()
    r = requests.get(
        url="https://api.twitter.com/1.1/search/tweets.json?geocode=37.78%2C-122.4167%2C25mi&result_type=recent&count=100",
        auth=oauth)
    max_id = int(generate_json(r.json())) - 1
    for x in range(70):
        r = requests.get(
            url="https://api.twitter.com/1.1/search/tweets.json?geocode=37.78%2C-122.4167%2C25mi&result_type=recent&count=100&max_id=" + str(
                max_id), auth=oauth)
        max_id = int(generate_json(r.json())) - 1
