...
tweets = json.load(open("../cs373-tweetCity/new_AUX.json"))
for tweet_id, info in tweets.items():
    ...
    for hashy in info["hashtags"]:
        if db.session.query(Hashtag).filter(Hashtag.text == hashy).all():
            data = Hashtag(hashy, "http://twitter.com/statuses/"+tweet_id)
            db.session.add(data)
